...
export default function Header({ 
  ...
  // Core nav items always visible on desktop (>=1024px)
  ...
  // Secondary nav items (collapsed into 'More' between 1024px and 1279px)
  const secondaryNavItems = [
    { id: 'deals', label: 'Deals', icon: Tag, screen: 'home' },
  ];

  return (
    <header className="sticky top-0 z-50 bg-white/95 dark:bg-[#14151A]/95 backdrop-blur-md border-b border-slate-200/80 dark:border-white/10 shadow-sm transition-colors duration-200">
        ...
        <div className="flex items-center justify-between h-16 gap-2">
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
            <button 
              ...
              className="flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-[#C30B12]/20 rounded-md py-1"
            ...
            {/* Sub-badge visible only on wider screens */}
            <div className="hidden xl:block h-5 w-px bg-slate-200 dark:bg-white/15" />
            <span className="hidden xl:inline-block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded">
              Official Booking
            </span>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-1 xl:gap-1.5" aria-label="Main Navigation">
            {primaryNavItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`h-10 px-2.5 xl:px-3.5 rounded-xl flex items-center gap-1.5 xl:gap-2 text-xs xl:text-[13.5px] font-medium transition-all duration-150 shrink-0 ${
                    isActive
              ...
              {secondaryNavItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    id={`header-nav-${item.id}`}
                    onClick={() => onNavigate(item.screen)}
                    className="h-10 px-3 rounded-xl flex items-center gap-1.5 text-[13.5px] font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100/80 dark:hover:bg-white/10 transition-colors shrink-0"
                  >
            ...
            <div className="relative shrink-0">
              <button
                ...
                className="h-10 px-2.5 xl:px-3 rounded-xl flex items-center gap-1.5 text-xs xl:text-[13.5px] font-medium text-amber-700 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-500/10 transition-colors"
              >
                <Crown className="w-4 h-4 text-[#F7941D] shrink-0" strokeWidth={1.8} />
                <span className="hidden sm:inline">SpiceClub</span>
                <ChevronDown className="w-3.5 h-3.5 text-amber-600/70 dark:text-amber-400/70" strokeWidth={2} />
              </button>
            ...
            {/* "More" Overflow Trigger (Active between 1024px and 1279px to prevent ANY header clipping) */}
            <div className="relative xl:hidden shrink-0">
              <button
                id="header-nav-more"
                onClick={() => setMoreMenuOpen(!moreMenuOpen)}
                className="h-10 px-2.5 rounded-xl border border-slate-200 dark:border-white/10 hover:border-slate-300 bg-slate-50 dark:bg-white/5 text-slate-700 dark:text-slate-300 text-xs font-medium flex items-center gap-1 transition-all"
                title="More options"
              >
          ...
          {/* Right Utility Bar: Theme Toggle + Currency + Help + Login/Signup CTA */}
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            
            {/* Theme Toggle Button (Light / Dark Mode per FIX 2) */}
            <button
              id="header-theme-toggle-btn"
              onClick={onToggleTheme}
              className="h-10 w-10 rounded-xl border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-white/5 text-slate-700 dark:text-slate-200 flex items-center justify-center transition-all shadow-resting hover:scale-105"
            ...
            <div className="relative shrink-0">
              <button
                ...
                className="h-10 px-2.5 sm:px-3 rounded-xl border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-white/5 text-slate-700 dark:text-slate-200 text-xs font-medium flex items-center gap-1.5 transition-all shadow-resting hover:bg-slate-50 dark:hover:bg-white/10"
            ...
            {/* Support / Help Button (Visible on xl: 1280px+) */}
            <button
              id="header-support-btn"
              onClick={() => alert('SpiceJet 24x7 Customer Support: +91 124 4983410 / custrelations@spicejet.com')}
              className="hidden xl:flex h-10 px-3 rounded-xl border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-white/5 text-slate-700 dark:text-slate-200 text-xs font-medium items-center gap-1.5 transition-all shadow-resting hover:bg-slate-50 dark:hover:bg-white/10"
            ...
            {/* Primary CTA: Login / Signup (NEVER collapsed or clipped - strict 40px height) */}
            <button
              id="header-login-btn"
              onClick={() => alert('SpiceClub Member Login modal')}
              className="h-10 px-3.5 sm:px-4 rounded-xl bg-[#C30B12] hover:bg-[#A8080E] active:bg-[#8F060B] dark:bg-[#FF3B46] dark:hover:bg-[#FF5A63] text-white text-xs font-bold flex items-center gap-1.5 sm:gap-2 transition-all shadow-sm hover:shadow-brand-glow shrink-0"
            >
